  ...
  return (
    <Card className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.25em] text-cyan-300">Intelligent commentary</p>
      ...
      <div className="space-y-3">
        {insights.map((insight) => (
          <article key={insight.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            {insight.evidence ? <p className="mt-2 text-xs leading-5 text-slate-400">Evidence: {insight.evidence}</p> : null}
            {insight.action ? <p className="mt-2 text-sm leading-6 text-cyan-100">Action: {insight.action}</p> : null}
          </article>
        ))}
      </div>
    </Card>
