  ...
  return (
    <Card className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.25em] text-brand-300">AI 코멘터리</p>
          <h3 className="mt-2 text-lg font-semibold text-white">{title}</h3>
        </div>
        <span className="rounded-full border border-brand-500/20 bg-brand-500/10 px-3 py-1 text-xs font-semibold text-brand-200">
          {assistantName}
        </span>
      </div>
      ...
      <div className="space-y-3">
        {insights.map((insight) => (
          <article key={insight.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            {insight.action ? <p className="mt-2 text-sm leading-6 text-brand-100">실행 항목: {insight.action}</p> : null}
          </article>
        ))}
      </div>
    </Card>
